...
- **Explicit login allowlist, not "== local user".** `desk` is a *tagged*
  ...
  `LoginName: desk.example.ts.net`), and no `tali@example.com` user
  exists until the phone joins. The allowlist must be typed:
  `tali@example.com` (whatever SSO login the phone ends up with; check the
...
2. Phone on the tailnet: either scan an invite (its origin is now
   `http://desk/`) — or tick **User-based auth**, enter `tali@example.com`,
