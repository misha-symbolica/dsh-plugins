...
1. **This node is not tagged** (`tailscale status --self --json`: `Tags: null`,
   user `tali@example.com`), and a request the node makes to *itself*
   through Serve carries `tailscale-user-login: tali@example.com` and
...
1. `~/.dsh/tailscale-remote.json`: `allowedUsers: ["tali@example.com"]`
