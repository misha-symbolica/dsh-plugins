...
let err = ''
try { await run('safari_evaluate', { expression: 'return 1' }) } catch (e) { err = String(e) }
...
const t0 = await run('safari_evaluate', { expression: 'return document.title', windowId: 's:0:0' })
const t1 = await run('safari_evaluate', { expression: 'return document.title', windowId: 's:0:1' })
...
const single = await run('safari_evaluate', { expression: 'return location.host' }); step('omitted windowId with 1 window → uses it', single.split('\n').join(' '))
...
const ev = await run('chrome_evaluate', { function: '() => document.title', windowId: 'c:0:0' }); step('chrome_evaluate c:0:0', ev.split('\n').slice(-1)[0])
