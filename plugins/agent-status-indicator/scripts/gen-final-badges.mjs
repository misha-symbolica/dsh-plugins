 ...
 * onto a modestly-rounded, tightly-packed rectangle ONLY when it declares a
 * `decoBg`; without one the deco stays a bare glyph (no filled box).
 * `decoTint` silhouettes use an alpha-type mask so the composed file stands
 * alone.
 *
...
`
  ...
  const bgRect = type.decoBg === undefined
    ? ''
    : `<rect x="${BOX.inset}" y="${BOX.inset}" width="${BOX.size}" height="${BOX.size}" rx="${BOX.radius}" fill="${type.decoBg}"/>`
...
for (const type of DOC_TYPES) {
  ...
  const rule = type.badge !== undefined
    ...
    : type.deco !== undefined
      ? `deco <code>${type.deco}</code>${type.decoBg !== undefined ? ` on <code>${type.decoBg}</code>` : ' (bare, no box)'}`
      : 'none'
  if (svg === undefined) {
...
modestly-rounded <code>decoBg</code>-coloured rectangle when one is declared, otherwise a bare glyph. Shown at 40px and at annotation size (20px).
...
// ── Runtime module: the same finals as bundled strings + extension lookup ──
const tsLines = [
  '/**',
  ' * GENERATED by scripts/gen-final-badges.mjs — do not edit by hand.',
  ' * Final annotation badges (standalone SVG markup, ids namespaced per type)',
  ' * plus the file-extension lookup, for annotating file-touching activity',
  ' * icons in the indicator.',
  ' */',
  '',
  '/** Final annotation-badge SVG markup by document-type id. */',
  'export const FINAL_BADGES: Readonly<Record<string, string>> = {',
  ...[...finalById.entries()].map(([id, svg]) => `  ${JSON.stringify(id)}: ${JSON.stringify(svg)},`),
  '}',
  '',
  '/** Lowercase file extension (no dot) -> document-type id with a final badge. */',
  'export const EXT_TO_TYPE: Readonly<Record<string, string>> = {',
  ...DOC_TYPES.filter(type => finalById.has(type.id))
    .flatMap(type => type.exts.map(ext => `  ${JSON.stringify(ext)}: ${JSON.stringify(type.id)},`)),
  '}',
  '',
]
writeFileSync('src/client/finalBadges.ts', tsLines.join('\n'))
console.log(`wrote src/client/finalBadges.ts (${finalById.size} badges)`)
