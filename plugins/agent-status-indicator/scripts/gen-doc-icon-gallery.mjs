 ...
 * solid rectangle with the type's decoration flush in the bottom-right corner.
 * When the type has a `decoBg`, that colour paints the ENTIRE document body
 * (instead of the default dark gray) and the unframed deco is overlaid
 * directly; otherwise the body stays dark gray with the bare deco, or the
 * framed badge for deco-less types (corner clipped to the document radius).
 * This function is the reference for what the plugin will do at runtime.
 */
function documentIcon(type) {
  const body = type.deco !== undefined && type.decoBg !== undefined ? type.decoBg : '#3a3a3f'
  let corner = ''
  if (type.deco !== undefined) {
    let glyph
    if (type.decoTint !== undefined) {
      const encoded = encodeURIComponent(svgText(type.deco)).replace(/'/g, '%27')
      const m = `url('data:image/svg+xml;utf8,${encoded}')`
...
<style>
  ...
  .doc { position:relative; width:34px; height:42px; border-radius:7px; }
...
<code>decoBg</code> chip when one is set) &middot; the document icon (dynamic composition: rounded rectangle —
<code>decoBg</code>-coloured body when set, else dark gray — decoration flush bottom-right, corner
clipped to the document radius) &middot;
